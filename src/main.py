...
from settings import (t, get_colors, set_lang, set_theme, get_lang, get_theme,
                       ...
                       LANGUAGES, THEMES, THEME_META, get_theme_name)
...
class Confirm:
    def __init__(s,parent,msg,on_yes):
        ...
        _btn(bf,t('confirm'),yes,bg=C['red']).pack(side='right')
    def close(s): s.ov.destroy(); s.fr.destroy()
...
class ContactsPage(tk.Frame):
    ...
    def _add(s):
        ...
        m.buttons(sv,t('add'))
    def _edit(s,c):
        m=Modal(s,t('contacts_edit_title'),400,420)
...
class LedgerPage(tk.Frame):
    def __init__(s,p,dm):
        ...
        s.tabs={}
        for txt,val in [(t('all'),None),(t('income'),'income'),(t('expense'),'expense')]:
        ...
        s.sf=ScrollFrame(s); s.sf.pack(fill='both',expand=True,padx=30,pady=(0,20))
        s.refresh()
    def _filter(s,t_): s.ft=t_; s.refresh()
    def refresh(s):
        for w in s.sf.inner.winfo_children(): w.destroy()
        ...
        s.slbl.config(text=t('summary').format(inc=fmt_money(inc),exp=fmt_money(exp),bal=fmt_money(inc-exp)))
        for k,b in s.tabs.items(): b.config(bg=C['acc'] if k==s.ft else C['card'])
        if not items: _lbl(s.sf.inner,t('ledger_empty'),12,fg=C['tx2']).pack(pady=40); return
    ...
    def _row(s,e):
        ...
        _lbl(tp,e.get('tag',''),11,True,bg=C['card']).pack(side='left')
        ...
        _btn(r,'x',lambda:Confirm(s,t('confirm_delete_short'),lambda:s._ddel(e['id'])),bg=C['red'],sz=9,px=6,py=2).pack(pady=(4,0))
    def _ddel(s,eid): s.dm.delete_ledger(eid); s.refresh()
    def _add(s):
        m=Modal(s,t('ledger_add_title'),440,580)
        _lbl(m.body,t('type'),10,fg=C['tx2'],bg=C['card']).pack(anchor='w',pady=(8,2))
        tv=tk.StringVar(value='expense')
        tf=tk.Frame(m.body,bg=C['card']); tf.pack(fill='x')
        for txt,val in [(t('income'),'income'),(t('expense'),'expense')]:
        ...
        def _build_tags():
            ...
            for i,tg in enumerate(s.dm.get_tags()):
                ...
                tk.Radiobutton(rf,text=tg,variable=tgv,value=tg,font=(FN,10),bg=C['card'],fg=C['tx'],selectcolor=C['acc'],activebackground=C['card']).pack(side='left')
                if not s.dm.is_default_tag(tg):
                    db=tk.Label(rf,text='x',font=(FN,8),bg=C['card'],fg=C['red'],cursor='hand2',padx=2)
                    db.pack(side='left')
                    db.bind('<Button-1>',lambda e,tag=tg:_del_tag(tag))
        def _del_tag(tag):
            s.dm.delete_tag(tag)
            if tgv.get()==tag: tgv.set('')
            _build_tags()
        _build_tags()
        _lbl(m.body,t('custom_tag'),10,fg=C['tx2'],bg=C['card']).pack(anchor='w',pady=(4,2))
        ct=_entry(m.body); ct.pack(fill='x',ipady=4)
        ed=m.field(t('description')); edt=m.field(t('date_format'),datetime.now().strftime('%Y-%m-%d'))
        def sv():
            try: amt=float(ea.get())
            except: m.warn(t('amount_error')); return
            tag=ct.get().strip() or tgv.get()
            if not tag: m.warn(t('tag_required')); return
...
class CalendarPage(tk.Frame):
    ...
    def refresh(s):
        ...
        for ri,week in enumerate(weeks):
            for ci,day in enumerate(week):
                ...
                if len(de)>2: etxts.append(t('event_count').format(n=len(de)-2))
    ...
    def _next(s):
        ...
        s.sel=None; s.refresh()
    def _jump(s):
        m=Modal(s,t('jump_date'),300,250)
        _lbl(m.body,t('year_label'),10,fg=C['tx2'],bg=C['card']).pack(anchor='w')
        ...
        _lbl(m.body,t('month_label'),10,fg=C['tx2'],bg=C['card']).pack(anchor='w',pady=(8,0))
    ...
    def _add_ev(s):
        ...
        tk.Checkbutton(m.body,text=t('allday'),variable=allday,font=(FN,11),bg=C['card'],fg=C['tx'],selectcolor=C['acc'],activebackground=C['card'],activeforeground=C['tx'],command=lambda:_tt()).pack(anchor='w',pady=(8,0))
        ...
        sd,st=_dr(t('start'),dd,'09:00'); ed,et2=_dr(t('end'),dd,'10:00')
        def _tt():
        ...
        def sv():
            if not et.get().strip(): m.warn(t('event_name_required')); return
            ss=sd.get(); ee=ed.get()
...
class DiaryPage(tk.Frame):
    ...
    def _card(s,e):
        ...
        if e.get('linked_events'): _lbl(bf,f"[{t('linked_ev').format(n=len(e['linked_events']))}]",9,fg=C['acc_l'],bg=C['card']).pack(side='left',padx=(0,6))
        if e.get('linked_ledger'): _lbl(bf,f"[{t('linked_led').format(n=len(e['linked_ledger']))}]",9,fg=C['grn'],bg=C['card']).pack(side='left',padx=(0,6))
        if e.get('linked_contacts'): _lbl(bf,f"[{t('linked_con').format(n=len(e['linked_contacts']))}]",9,fg=C['blu'],bg=C['card']).pack(side='left')
        r=tk.Frame(f,bg=C['card']); r.pack(side='right',padx=12,pady=8)
    ...
    def _editor(s,ex):
        ...
        if events:
            _lbl(inner,t('link_events'),10,fg=C['tx2']).pack(anchor='w',pady=(8,2))
        ...
        if ledger:
            _lbl(inner,t('link_ledger'),10,fg=C['tx2']).pack(anchor='w',pady=(8,2))
            ll=ex.get('linked_ledger',[]) if ex else []
            for le in ledger[:6]:
                v=tk.BooleanVar(value=le['id'] in ll)
                sg='+' if le['type']=='income' else '-'
                tk.Checkbutton(inner,text=f"{le.get('date','')} {le.get('tag','')} {sg}{fmt_money(le['amount'])}",variable=v,font=(FN,10),bg=C['bg'],fg=C['tx'],selectcolor=C['acc'],activebackground=C['bg']).pack(anchor='w')
                lev[le['id']]=v
        cov={}; contacts=s.dm.get_contacts()
        if contacts:
            _lbl(inner,t('link_contacts'),10,fg=C['tx2']).pack(anchor='w',pady=(8,2))
        ...
        def sv():
            if not et.get().strip():
                ...
                wl[0]=_lbl(inner,t('title_required'),10,fg=C['red']); wl[0].pack(anchor='w'); return
...
class ReminderPage(tk.Frame):
    ...
    def _card(s,r,now):
        ...
        _lbl(tp,r.get('title',''),14,True,fg=C['tx2'] if done else C['tx'],bg=C['card']).pack(side='left')
        try:
            dl=datetime.strptime(r['deadline'][:10],'%Y-%m-%d'); diff=(dl-now).days
            if done: dt,dc=t('done'),C['grn']
            elif diff<0: dt,dc=t('overdue').format(d=abs(diff)),C['red']
            elif diff==0: dt,dc=t('dday'),C['yel']
            else: dt,dc=f'D-{diff}',C['acc_l']
        except: dt,dc='',C['tx2']
        _lbl(tp,dt,11,True,fg=dc,bg=C['card']).pack(side='right')
        _lbl(l,f"{t('deadline_prefix')}: {r.get('deadline','')}",10,fg=C['tx2'],bg=C['card']).pack(anchor='w')
        ...
        _btn(rt,t('edit'),lambda:s._edit(r),bg=C['hover'],sz=10,px=10,py=4).pack(pady=2)
        _btn(rt,t('delete'),lambda:Confirm(s,t('confirm_delete_short'),lambda:s._ddel(r['id'])),bg=C['red'],sz=10,px=10,py=4).pack(pady=2)
    def _tog(s,rid): s.dm.toggle_reminder(rid); s.refresh()
    def _ddel(s,rid): s.dm.delete_reminder(rid); s.refresh()
    def _add(s):
        m=Modal(s,t('reminder_add_title'),420,360)
        et=m.field(t('title')); ed=m.field(t('deadline'),datetime.now().strftime('%Y-%m-%d 23:59'))
        edesc=m.field(t('description'))
        def sv():
            if not et.get().strip(): m.warn(t('title_required')); return
            s.dm.add_reminder(et.get(),ed.get(),edesc.get()); m.close(); s.refresh()
        m.buttons(sv,t('add'))
    def _edit(s,r):
        m=Modal(s,t('reminder_edit_title'),420,360)
        et=m.field(t('title'),r.get('title','')); ed=m.field(t('deadline_label'),r.get('deadline',''))
        edesc=m.field(t('description'),r.get('description',''))
        def sv():
...
class SettingsPage(tk.Frame):
    def __init__(s,p,dm,app):
        ...
        s.sel_lang=get_lang(); s.sel_theme=get_theme()
        s.sf=ScrollFrame(s); s.sf.pack(fill='both',expand=True,padx=30,pady=(0,20))
        s._build()
    def _build(s):
        inner=s.sf.inner
        ...
        for i,(code,name) in enumerate(LANGUAGES.items()):
            ...
            rb=tk.Radiobutton(bf,text=name,variable=s.lang_var,value=code,
                              font=(FN,11),bg=C['card'],fg=C['tx'],
                              selectcolor=C['acc'],activebackground=C['card'],activeforeground=C['tx'],
                              command=lambda c=code:s._pick_lang(c))
            rb.pack(side='left')
            if code==get_lang():
                tag=tk.Label(bf,text=f"  ✓ {t('current')}",font=(FN,9),bg=C['card'],fg=C['acc_l'])
                tag.pack(side='left')
    ...
    def _build_themes(s):
        ...
        for i,tkey in enumerate(THEMES):
            cols=THEMES[tkey]
            row,col=divmod(i,4)
            card=tk.Frame(s.theme_fr,bg=C['card'],highlightthickness=2,
                          highlightbackground=C['acc'] if tkey==s.sel_theme else C['brd'],
                          cursor='hand2')
            ...
            for j,ck in enumerate(['acc','grn','red','blu']):
                sw=tk.Frame(bar,bg=cols[ck],width=20,height=20)
                sw.pack(side='left',padx=2)
                sw.pack_propagate(False)
            # Card body preview
            body_prev=tk.Frame(card,bg=cols['card'],height=14); body_prev.pack(fill='x',padx=8,pady=2)
            # Theme name
            nm=get_theme_name(tkey)
            nl=tk.Label(card,text=nm,font=(FN,10,'bold' if tkey==s.sel_theme else 'normal'),
                        bg=C['card'],fg=C['acc'] if tkey==s.sel_theme else C['tx'],padx=8,pady=6)
            nl.pack()
            if tkey==get_theme():
                tag=tk.Label(card,text=f"✓ {t('current')}",font=(FN,8),bg=C['card'],fg=C['acc_l'])
                tag.pack(pady=(0,4))
            # Bind click
            for w in [card,prev,bar,body_prev,nl]:
                w.bind('<Button-1>',lambda e,k=tkey:s._pick_theme(k))

    def _pick_lang(s,code):
        s.sel_lang=code
    def _pick_theme(s,key):
        s.sel_theme=key; s._build_themes()
    def _apply(s):
        settings={'language':s.sel_lang,'theme':s.sel_theme}
        s.dm.save_settings(settings)
        set_lang(s.sel_lang); set_theme(s.sel_theme)
        _apply_colors()
        s.app.rebuild()
...
class PersonalManager:
    def __init__(s):
        ...
        set_lang(st.get('language','ko')); set_theme(st.get('theme','dark_purple'))
        _apply_colors()
        s.mbtns=[]; s.cur=None; s.sb_fr=None
        s._sb()
        s.content=tk.Frame(s.root,bg=C['bg']); s.content.pack(side='right',fill='both',expand=True)
        s.show('contacts')
    def rebuild(s):
        """Rebuild entire UI after settings change"""
        s.root.configure(bg=C['bg'])
        if s.sb_fr: s.sb_fr.destroy()
    ...
    def _sb(s):
        ...
        _lbl(s.sb_fr,'Personal\nManager',16,True,bg=C['sb']).pack(pady=(28,24))
        tk.Frame(s.sb_fr,bg=C['brd'],height=1).pack(fill='x',padx=16,pady=(0,12))
        for key,tkey in [('contacts','nav_contacts'),('ledger','nav_ledger'),('calendar','nav_calendar'),('diary','nav_diary'),('reminder','nav_reminder'),('settings','nav_settings')]:
    ...
    def show(s,key):
        ...
        if key=='settings':
            SettingsPage(s.content,s.dm,s).pack(fill='both',expand=True)
        else:
